...
if (array_search($extensao, $_UP['extensoes']) === false) {
	...
	if ($_UP['renomeia'] == true) {
		$nome_final = '1logom_'.$_SESSION['login'].'.jpg';
	} else {
		$nome_final = $_FILES['arquivo']['name'];
	}
	if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'] . $nome_final)) {
		if (file_exists("plugins/scsus/temas/".$_SESSION['tema']."/img/logom_".$_SESSION['login'].".jpg")){
			unlink("plugins/scsus/temas/".$_SESSION['tema']."/img/logom_".$_SESSION['login'].".jpg");
		}
		if (file_exists("plugins/scsus/temas/".$_SESSION['tema']."/img/1logom_".$_SESSION['login'].".jpg")){
			rename("plugins/scsus/temas/".$_SESSION['tema']."/img/1logom_".$_SESSION['login'].".jpg", "plugins/scsus/temas/".$_SESSION['tema']."/img/logom_".$_SESSION['login'].".jpg");
