...
// -----------------------------------------------------------------------
$file = "html/rel_criancas_".$_SESSION['login'].".html";
if (file_exists($file)){unlink($file);}
$HTML = fopen($file,'w');
// -----------------------------------------------------------------------
$file = "csv/criancas_T_".$_SESSION['login'].".csv";
if (file_exists($file)){unlink($file);}
$FT = fopen($file,'w');
$texto = "Seq;CPF;CNS;Nome;DtNascimento;Mae;Idade;MarcGestante;MarcHipertensa;MarcDiabetica;Indicador5;CNES;INE;MA\r\n";
fwrite($FT, $texto);
// -----------------------------------------------------------------------
$file = "csv/criancas_V_".$_SESSION['login'].".csv";
if (file_exists($file)){unlink($file);}
$FV = fopen($file,'w');
...
if ($paginacao <= 0 || $nm_sql_2 == 0){
	...
	$file = "resumo/r_ind5_".$_SESSION['login'].".php";
	if (file_exists($file)){unlink($file);}
	$FIN = fopen($file,'w');
	$porc_ind = 0;
