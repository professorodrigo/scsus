...
// -----------------------------------------------------------------------
$file = "html/rel_mulheres_".$_SESSION['login'].".html";
if (file_exists($file)){unlink($file);}
$HTML = fopen($file,'w');
// -----------------------------------------------------------------------
$file = "csv/mulheres_T_".$_SESSION['login'].".csv";
if (file_exists($file)){unlink($file);}
$FT = fopen($file,'w');
$texto = "Seq;CPF;CNS;Nome;DtNascimento;Mae;Idade;MarcGestante;MarcHipertensa;MarcDiabetica;Indicador4;CNES;INE;MA\r\n";
fwrite($FT, $texto);
// -----------------------------------------------------------------------
$file = "csv/mulheres_P_".$_SESSION['login'].".csv";
if (file_exists($file)){unlink($file);}
$FP = fopen($file,'w');
...
if (($paginacao <= 0 && $ridade <= 0) || $nm_sql_2 == 0){
	...
	$file = "resumo/r_ind4_".$_SESSION['login'].".php";
	if (file_exists($file)){unlink($file);}
	$FIN = fopen($file,'w');
	$porc_ind = 0;
