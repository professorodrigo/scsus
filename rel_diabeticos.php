...
if (file_exists("config/c_rel_d_".$_SESSION['key'].".php")){
	require_once("config/c_rel_d_".$_SESSION['key'].".php");
} else {
	$mensagem = "
	  <script type=\"text/javascript\">
		$(document).ready(function() {
...
// -----------------------------------------------------------------------
$file = "html/rel_diabeticos_".$_SESSION['key'].".html";
if (file_exists($file)){unlink($file);}
$HTML = fopen($file,'w');
// -----------------------------------------------------------------------
$file = "csv/diabeticos_T_".$_SESSION['key'].".csv";
if (file_exists($file)){unlink($file);}
$FT = fopen($file,'w');
$texto = "Seq;CPF;CNS;Nome;DtNascimento;Mae;Idade;MarcGestante;MarcHipertenso;MarcDiabetico;Indicador7;Sexo;CNES;INE;MA;NumConsS1;NumConsS2;NumProcS1;NumProcS2;NumExSol\r\n";
fwrite($FT, $texto);
// -----------------------------------------------------------------------
$file = "csv/diabeticos_C_".$_SESSION['key'].".csv";
if (file_exists($file)){unlink($file);}
$FC = fopen($file,'w');
$texto = "Seq;CPF;CNS;DtProced;CNES;INE;CBO;Semestre;CID;CIAP;CNSProf;NomeProf;ProfAlerta\r\n";
fwrite($FC, $texto);
// -----------------------------------------------------------------------
$file = "csv/diabeticos_P_".$_SESSION['key'].".csv";
if (file_exists($file)){unlink($file);}
$FP = fopen($file,'w');
...
$tabela_temp = "
CREATE TEMPORARY TABLE tmp_diabeticos (
...
if ($nm_sql_1 > 0){
	while ($result1 = pg_fetch_array($run_s1)){
		...
		if ($busca){
			...
			$cidadao_cpf = '0';
			$cidadao_cns = '0';
			$cidadao_nome = "# DIABETICO NÃO ENCONTRADO #";
...
<table width=\"1009\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" class=\"relatorio-tabela\">
  <!--DWLayoutTable-->
  <tr> 
    <td width=\"440\" height=\"32\" valign=\"top\" class=\"relatorio-titulo\">Relatório de Diabéticos</td>
...
if ($nm_sql_2 > 0){
	while ($result2 = pg_fetch_array($run_hi)){
		...
		$procedimentos = "'0202010503','ABEX008'";
		$apenas_solicitados = 0;
		// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		//   (BUSCA) tabela 1
		// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		$select_tb1 = "
		...
		if ($ma_familiar == '00' && $result2['cind_micro_area'] == '00'){
			$inconsistencias .= " diabético sem cadastro familiar.<br>";
		}

		if (substr($result2['cidadao_nome'],0,1) == '#'){
			$inconsistencias .= "diabético não localizada em tb_cidadao.<br>";
		}
		$inativo2 = 0;
		...
		if (count($array_procedimentos) > 0){
			...
			//$soma_proc_x = $soma_proc_s1 + $soma_proc_s2;
			//if ($conta_consultas_x > 0 && $soma_proc_x > 0){
			if ($conta_consultas_x > 0 && $apenas_solicitados > 0){	
				$exame_rotina = "SIM";
		...
		$rel_dados_unitario = "
		  ...
		  <tr> 
			<td height=\"19\"></td>
			<td></td>
			<td colspan=\"2\" valign=\"top\" class=\"lista-sub-dados-BB\">Exames Solicitados</td>
			<td colspan=\"2\" valign=\"top\" class=\"lista-dado2-centro-B\">".$apenas_solicitados."</td>
		  ...
		  <tr> 
			<td height=\"16\"></td>
			<td></td>
			<td colspan=\"2\" rowspan=\"2\" valign=\"top\" class=\"".$estilo_exrot_indicador."\">Indicador 7 [ ".$exame_rotina." ]</td>
			<td colspan=\"3\" rowspan=\"2\" valign=\"top\" class=\"indicador-c1-B\">Hemoglobina glicada</td>
			  ...
			  <tr> 
				...
				<table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" class=\"procedimentos-tabela\">
					...
					<tr> 
					  ...
					  <td width=\"213\" valign=\"top\" class=\"procedimentos-cabeca\">.</td>
...
if ($paginacao > 0){
	...
	if ($nm_sql_2 > 0){
		$link_pag = "<a href=\"rel_diabeticos.php?pini=".$pini."&num=".$num."&cdes=".$cdes."\"><img src=\"plugins/scsus/temas/".$_SESSION['tema']."/img/pagina.png\"></a>";
	}
}
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
if ($conta_geral_desconto > 0){
	$show_desconto = " (descontar ".$conta_geral_desconto." diabetico(s) por conta de inconsistências)";
}
...
<table width=\"1009\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" class=\"rodape-tabela\">
  ...
  <tr> 
	...
	".$sobre['versao']." | 
	".date('d/m/Y')." | 
	Total de ".$total_geral." diabeticos | 
...
if ($paginacao <= 0 || $nm_sql_2 == 0){
	...
	$file = "resumo/r_ind7_".$_SESSION['key'].".php";
	if (file_exists($file)){unlink($file);}
	$FIN = fopen($file,'w');
	$porc_ind = 0;
	...
	$txind = "<?php
	\$den_ind_7 = ".$denominador_final.";
	\$den_est_7 = ".$des.";
	\$num_7 = ".$numerador_ind7.";
	\$num_prc_7 = ".$porc_ind.";
	\$num_prc_est_7 = ".$porc_ind_est.";
	\$quadri_7 = \"".qdata($dti)."\";
	\$versao_7 = \"".$sobre['versao']."\";
	\$banco_7 = \"".$dbdb."\";
	\$datahora_7 = \"".date('d/m/Y H:i:s')."\";
	\$dti_7 = ".$dti.";
	\$dtf_7 = ".$dtf.";
	\$incons_7 = ".$conta_geral_desconto.";
	\$c_hiper_7 = ".$contator_gl_hipertenso.";
	\$c_gest_7 = ".$contator_gl_gestante.";
	";
	for ($e=0;$e<count($total_equipe);$e++){
		$txind .= "\r\$equipe_7[".$e."]['ine'] = \"".$total_equipe[$e]['ine']."\"; \$equipe_7[".$e."]['nome'] = \"".$total_equipe[$e]['nome']."\"; \$equipe_7[".$e."]['den'] = \"".$total_equipe[$e]['den']."\"; \$equipe_7[".$e."]['num'] = \"".$total_equipe[$e]['num']."\";";
	}
	for ($e=0;$e<count($total_unidade);$e++){
		$txind .= "\r\$unidade_7[".$e."]['cnes'] = \"".$total_unidade[$e]['cnes']."\"; \$unidade_7[".$e."]['nome'] = \"".$total_unidade[$e]['nome']."\"; \$unidade_7[".$e."]['den'] = \"".$total_unidade[$e]['den']."\"; \$unidade_7[".$e."]['num'] = \"".$total_unidade[$e]['num']."\";";
	}
	$txind .= "
	?>\r\n";
	fwrite($FIN, $txind);
	...
	<table width=\"1009\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
	  ...
	  <tr> 
		...
		<td width=\"260\" valign=\"top\" class=\"resumo-sub-titulo\">Indicador 7</td>
		<td width=\"120\">&nbsp;</td>
	  </tr>
	  <tr> 
		<td height=\"30\" valign=\"top\" class=\"resumo-numerador\">Numerador&nbsp;&nbsp;</td>
		<td valign=\"top\" class=\"resumo-v-numerador\">".$numerador_ind7." [ ".ceil($porc_ind)."% ]</td>
		<td></td>
	  </tr>
	  <tr> 
